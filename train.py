...
@torch.no_grad()
def eval_train_acc(epoch=0, tb=True):
    ...
    #add informations to tensorboard
    if tb:
        writer.add_scalar('Train/Average loss', train_loss / len(cifar100_training_loader.dataset), epoch)
        writer.add_scalar('Train/Accuracy', correct.float() / len(cifar100_training_loader.dataset), epoch)

    return correct.float() / len(cifar100_training_loader.dataset)
...
if __name__ == '__main__':
    ...
    #data preprocessing:
    _, cifar100_training_loader = get_training_dataloader(
    ...
    # train_scheduler = optim.lr_scheduler.MultiStepLR(optimizer, milestones=settings.MILESTONES, gamma=0.6) #learning rate decay
    train_scheduler = optim.lr_scheduler.ExponentialLR(optimizer, gamma=0.9623)
    ...
    for epoch in range(1, settings.EPOCH + 1):
        ...
        # if epoch % 20 == 0:
        #     train_acc = eval_train_acc(epoch)
